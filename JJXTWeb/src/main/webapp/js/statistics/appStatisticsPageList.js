$(function() {
	...
	$('#mytab').bootstrapTable({
		...
		url : "findAppStatisticsList.action",// 请求后台url
		...
		onLoadSuccess: function (data) {
		    ...
		    if(passed("显示优享")) {
		    	$('#mytab').bootstrapTable('showColumn','shuntCharge');
		    	$('#mytab').bootstrapTable('showColumn','shuntRate');
		    }else{
		    	$('#mytab').bootstrapTable('hideColumn','shuntCharge');
		    	$('#mytab').bootstrapTable('hideColumn','shuntRate');
		    }
		    ...
		    if(passed("隐藏提交成功")) {
		    	$('#mytab').bootstrapTable('hideColumn','sendSuccessCharge');
		    }else{
		    	$('#mytab').bootstrapTable('showColumn','sendSuccessCharge');
		    }
	 ...
	 $("#search_btn").click(function () {
		 $('#mytab').bootstrapTable('refresh',{url:'findAppStatisticsList.action'});
		});
...
function channelDetail(value, row, index) {
	var channelId = value == '0' ? '':value;
	var result = "<a href='javascript:;' data-toggle='modal' data-target='#chanDetails' onclick=\"findChanDetails('"+ row.channelId +"')\" title='详情'>"+ channelId +"</a>";
	return result;
}
...
function findAppDetails(appId) {
	$.ajax({
		...
		success:function (result){
			...
			$("#productIdName").html(result.productId +" : "+ result.productName);
...
function findChanDetails(channelId) {
	$.ajax({
		url:"findChanDetailById.action",
		data:{channelId:channelId},
		dataType:"json",
		type:"post",
		success:function (result){
			$("#channelName").html(result.channelId+" : "+result.channelName);
			if(result.channelStatus == 'normal'){
    			$("#channelStatus").html("正常");
    		}else if(result.channelStatus == 'paused'){
    			$("#channelStatus").html("暂停");
    		}else if(result.channelStatus == 'deleted'){
    			$("#channelStatus").html("删除");
    		}
			$("#channelSpeed").html(result.channelSpeed);
			$("#channelPrice").html(result.channelPrice);
		}
	});
}
