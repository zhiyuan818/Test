$(function() {
	...
	$(window).resize(function() {
		$('#keywordtab').bootstrapTable('resetView', {
			height : tableHeight()
		})
	})

	$('#keywordtab').bootstrapTable({
		method : 'post',
	...
	$("#search_btn").click(function() {
		$('#keywordtab').bootstrapTable('refreshOptions', {
			pageNumber : 1
		});
	});
...
$(function() {
	...
	$('#add_saveBtn').click(
			function() {
				...
				if ($("#addForm").data('bootstrapValidator').isValid()) {
					...
					$.post("addToBlackKeyword.action", data1, function(data) {
						// 后台返回添加成功
						if (data.result > 0) {
							$('.addBody').addClass('animated slideOutLeft');
							setTimeout(function() {
							...
							$('#addForm').data('bootstrapValidator').resetForm(
									true);
							$('#keywordtab').bootstrapTable('refresh', {
								url : 'findPageList.action'
							});
						} else {
		...
		mark=true;
				...
				if ($("#editForm").data('bootstrapValidator').isValid()) {
					...
					$('.popup_de .btn_submit').one('click',function() {
						...
						if(mark){
							$.post("updateBlackKeyword.action",
									data,
									function(data) {
								if (data.result == 1) {
									...
									$('#keywordtab').bootstrapTable('refresh',{url : 'findPageList.action'});
	...
	$('#btn_del').click(function() {
		mark=true;
		if (passed('批量删除')) {
			var dataArr = $('#keywordtab').bootstrapTable('getSelections');
			...
			$('.popup_de .btn_submit').one('click', function() {
				...
				if (ID.length > 0) {
					if(mark){
						$.post("delBlackKeywordBatch.action", {
							ids : ID
						}, function(data) {
							if (data.result == 1) {
	...
	$('.table').change(
			function() {
				var dataArr = $('#keywordtab .selected');
				if (dataArr.length >= 1) {
...
// 修改
function updateBlackKeyWord(id) {
	if (passed('修改')) {
		$.post("findBlackKeywordById.action", {
			id : id
		}, function(result) {
			$('.tableBody').addClass('animated slideOutLeft');
			setTimeout(function() {
...
function actionFormatter(value, row, index) {
	var id = value;
	var result = "";
	result += "<button class='btn btn-primary btn-xs' id='update_btn' onclick=\"updateBlackKeyWord('"
			+ row.id + "')\" title='点击修改'>修改</button>&nbsp;&nbsp;";
	result += "<button class='btn btn-primary btn-xs' id='delete_btn' onclick=\"delBlackKeyWord('"
